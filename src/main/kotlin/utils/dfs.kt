private enum class GateA { AND, OR, XOR, INPUT }
private data class Wire(var name: String, var value: Boolean?, val gate: GateA, val input1: String?, val input2: String?)
...
private fun getCircuit(input: String): Circuit {
    val circuit = Regex("""(\w+): (\d)""").findAll(input).map {
        ...
        Wire(name, value == "1", GateA.INPUT, null, null)
    }.toMutableList()

    Regex("""(\w+) (AND|OR|XOR) (\w+) -> (\w+)""").findAll(input).forEach {
        val (input1, gate, input2, output) = it.destructured
        circuit.add(Wire(output, null, GateA.valueOf(gate), input1, input2))
    }
...
private fun getWireValue(circuit: Circuit, wire: Wire): Boolean {
    ...
    val value = when (wire.gate) {
        GateA.INPUT -> wire.value
        GateA.AND -> getWireValue(circuit, input1!!) and getWireValue(circuit, input2!!)
        GateA.OR -> getWireValue(circuit, input1!!) or getWireValue(circuit, input2!!)
        GateA.XOR -> getWireValue(circuit, input1!!) xor getWireValue(circuit, input2!!)
    }
...
fun solveDay24(input: String): String {
    val circuit = getCircuit(input)
    val invalidEndWires = circuit.filter {
        it.name.first() == 'z' && it.name != "z45" && it.gate != GateA.XOR
    }
...
fun main() {
    val input = Any::class::class.java.getResource("/y2024/day24.txt")?.readText()!!
    println(solveDay24(input))
}